...
include AppraisalCyclesHelper
  ...
  def dr_performance_graph
      @dr_sheet=DrSheet.where(:appraisee_id => current_user.id,:appraisee_status=>1,:appraiser_status=>1).last
    unless @dr_sheet.nil?
      @dr_ratings_by_manager_array=DrRating.where(:dr_sheet_id => @dr_sheet.id, :rated_by => 1).select(:rating).map(&:rating)
      @dr_ratings_by_self_array=DrRating.where(:dr_sheet_id => @dr_sheet.id, :rated_by => 0).select(:rating).map(&:rating)
      @dr_ratings_by_self_array.map! { |x| x == nil ? 0 : x }
      @dr_ratings_by_manager_array.map! { |x| x == nil ? 0 : x }
      @dr_list=DrRating.where(:dr_sheet_id => @dr_sheet.id, :rated_by => 1)
